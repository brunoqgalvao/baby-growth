 ...
 *   --aspect-ratio, -ar <ratio>             Aspect ratio (1:1, 16:9, 9:16, 4:3, 3:4, etc.)
 */
...
function parseArgs(args) {
  ...
  while (i < args.length) {
    ...
    } else if (arg === "--aspect-ratio" || arg === "-ar") {
...
Options:
  ...
  --aspect-ratio, -ar  Aspect ratio (1:1, 16:9, 9:16, 4:3, 3:4, 3:2, 2:3, 4:5, 5:4, 21:9)
...
async function generateImage(prompt, usePro, outputPath, aspectRatio = null) {
  ...
  if (aspectRatio) {
    configObj.imageConfig = { aspectRatio };
  }
...
async function editImage(imagePath, prompt, usePro, outputPath, aspectRatio = null) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    console.error("Error: GEMINI_API_KEY not found in .env file");
    process.exit(1);
...
async function combineImages(imagePaths, prompt, usePro, outputPath, aspectRatio = null) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    console.error("Error: GEMINI_API_KEY not found in .env file");
    process.exit(1);
...
async function main() {
  ...
  try {
    if (parsed.command === "generate") {
      ...
      await generateImage(parsed.prompt, parsed.usePro, parsed.output, parsed.aspectRatio);
      ...
      await editImage(parsed.imagePath, parsed.prompt, parsed.usePro, parsed.output, parsed.aspectRatio);
      ...
      await combineImages(parsed.imagePaths, parsed.prompt, parsed.usePro, parsed.output, parsed.aspectRatio);
    } else {
