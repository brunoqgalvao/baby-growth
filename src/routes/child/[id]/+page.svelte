<script lang="ts">
	...
	let editNameValue = $state(data.child.name);
	...
	function resizeImage(file: File, maxSize = 400, quality = 0.7): Promise<Blob> {
	...
	async function handlePhotoChange(e: Event) {
		...
		try {
			...
			childPhoto = URL.createObjectURL(compressed);
...
<div class="max-w-4xl mx-auto px-4 py-8 pb-24 sm:pb-8 animate-in">
	...
	<div class="flex items-center gap-4 mb-8 flex-wrap">
		...
		<div>
			<div class="flex items-center gap-1.5">
				{#if editingName}
					<form method="POST" action="?/updateChildName" use:enhance={() => {
						savingName = true;
						...
						<input
							...
							class="text-2xl font-bold text-[var(--cream-700)] bg-white border border-[var(--cream-300)] rounded-lg px-2 py-0.5 outline-none focus:border-[var(--primary)] transition-colors w-48"
							autofocus
							onkeydown={(e) => { if (e.key === 'Escape') { editingName = false; editNameValue = child.name; } }}
						/>
						<button type="submit" disabled={savingName} class="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--mint-500)] hover:bg-[var(--mint-100)] transition-all cursor-pointer disabled:opacity-50" title="Save">
							{#if savingName}
						...
						<button type="button" onclick={() => { editingName = false; editNameValue = child.name; }} class="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--cream-400)] hover:bg-[var(--cream-100)] transition-all cursor-pointer" title="Cancel">
				...
				{:else}
					...
					<button
						onclick={() => { editNameValue = child.name; editingName = true; }}
						class="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--cream-300)] hover:text-[var(--cream-600)] hover:bg-[var(--cream-100)] transition-all cursor-pointer"
						title="Edit name"
					>
	...
	{#if showInvitePanel}
		<div class="card p-5 mb-6 animate-in">
			...
			{#if data.sharedWith.length > 0}
				<div class="mb-4 p-3 bg-[var(--mint-50)] rounded-[var(--radius-sm)] border border-[var(--mint-200)]">
					<p class="text-xs font-semibold text-[var(--cream-600)] mb-2 uppercase tracking-wider">Shared with</p>
					<div class="flex flex-col gap-2">
						{#each data.sharedWith as partner}
							<div class="flex items-center gap-2.5">
								<div class="w-7 h-7 rounded-full bg-[var(--mint-100)] flex items-center justify-center shrink-0">
									<svg class="w-3.5 h-3.5 text-[var(--mint-500)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
								</div>
								<div class="min-w-0">
									<p class="text-sm font-medium text-[var(--cream-700)] truncate">{partner.name || partner.email}</p>
									{#if partner.name}
										<p class="text-xs text-[var(--cream-400)] truncate">{partner.email}</p>
									{/if}
								</div>
								<svg class="w-4 h-4 text-[var(--mint-400)] ml-auto shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
							</div>
						{/each}
					</div>
				</div>
	...
	<!-- Share with partner CTA (mobile only — desktop has share in header) -->
	<div class="card p-5 mb-5 flex items-center gap-4 sm:hidden">
		<div class="w-10 h-10 rounded-full {data.sharedWith.length > 0 ? 'bg-[var(--mint-50)]' : 'bg-[var(--peach-50)]'} flex items-center justify-center shrink-0">
			{#if data.sharedWith.length > 0}
				<svg class="w-5 h-5 text-[var(--mint-500)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
			{:else}
		...
		<div class="flex-1 min-w-0">
			{#if data.sharedWith.length > 0}
				<p class="text-sm font-bold text-[var(--cream-700)]">Shared with {data.sharedWith.map(p => p.name || p.email).join(', ')}</p>
				<p class="text-xs text-[var(--cream-500)]">Tap to manage sharing or invite more</p>
			{:else}
		...
		<button
			onclick={() => showShareDrawer = true}
			class="{data.sharedWith.length > 0 ? 'btn-secondary' : 'btn-primary'} px-4 py-2 text-sm shrink-0 flex items-center gap-1.5"
		>
			<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
				<path stroke-linecap="round" stroke-linejoin="round" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"/>
			</svg>
			{data.sharedWith.length > 0 ? 'Manage' : 'Share'}
		</button>
	</div>
...
<!-- Share drawer (mobile bottom sheet) -->
{#if showShareDrawer}
	<!-- svelte-ignore a11y_click_events_have_key_events a11y_interactive_supports_focus -->
	<div
		class="fixed inset-0 bg-black/20 z-50 backdrop-blur-[2px]"
		onclick={(e) => { if (e.target === e.currentTarget) showShareDrawer = false }}
		role="dialog"
		style="animation: fadeOverlay 0.2s ease-out"
	>
		<div class="measurement-sheet absolute bottom-0 left-0 right-0 bg-white rounded-t-[24px] shadow-[0_-8px_40px_rgba(0,0,0,0.12)] max-h-[85vh] overflow-y-auto" style="animation: slideUp 0.3s cubic-bezier(0.16, 1, 0.3, 1)">
			<!-- Drag handle -->
			...
			<div class="px-5 pb-6 pt-2">
				...
				<!-- Shared with list (mobile drawer) -->
				{#if data.sharedWith.length > 0}
					<div class="mb-4 p-3 bg-[var(--mint-50)] rounded-[var(--radius-sm)] border border-[var(--mint-200)]">
						<p class="text-xs font-semibold text-[var(--cream-600)] mb-2 uppercase tracking-wider">Shared with</p>
						<div class="flex flex-col gap-2">
				...
				{:else}
					...
					<form method="POST" action="?/createInvite" use:enhance={() => {
						...
						<button type="submit" disabled={generatingInvite} class="btn-primary w-full py-3 text-sm flex items-center justify-center gap-2 disabled:opacity-60">
							{#if generatingInvite}
								<svg class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg>
								Generating...
							{:else}
